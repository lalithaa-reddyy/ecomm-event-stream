...
exports.handler = async (event) => {
  ...
  try {
    ...
    const rate = body.rate || 50000;          // events per minute (minimum 50,000)
    ...
    if (action === "start") {
      ...
      console.log(`🚀 Starting stream: ${rate}/min for ${duration}s with temporal variance (0.7-1.3)`);
      streamingActive = true;

      // Calculate events per second with temporal variance
      ...
      return {
        statusCode: 200,
        body: JSON.stringify({ message: "⏹️  Stream stopped" })
      };
